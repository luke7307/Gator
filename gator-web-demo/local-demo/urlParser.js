const URL_PARSER = {
    ...
    async fetchPageContent(url, timeout = 15000) {
        try {
            ...
            // 优先使用 Electron 主进程的 fetchUrl（最稳定）
            if (window.electronAPI && window.electronAPI.fetchUrl) {
                try {
                    ...
                    console.warn('electronAPI.fetchUrl 失败，尝试代理服务器:', e.message);
                }
            }
            
            // 如果 Electron API 失败，尝试本地代理服务器
            if (!html || html.length < 50) {
                const tryProxy = async (proxyUrl) => {
                    try {
                        ...
                        console.log('代理尝试失败:', e.message, proxyUrl.substring(0, 50));
                    }
                    return '';
                };
                
                // 先尝试固定端口 3000
                const proxy3000 = 'http://127.0.0.1:3000/fetch?url=' + encodeURIComponent(url);
                html = await tryProxy(proxy3000);
                
                // 如果有动态端口，再试动态端口
                if (!html && window.electronAPI && window.electronAPI.getProxyPort) {
                    try {
                        const port = await window.electronAPI.getProxyPort();
                        if (port && port !== 3000) {
                            const dynamicProxy = `http://127.0.0.1:${port}/fetch?url=${encodeURIComponent(url)}`;
                            html = await tryProxy(dynamicProxy);
                        }
                    } catch (e) {
                        console.log('获取代理端口失败:', e.message);
                    }
                }
    ...
    cleanWechatContent: function(html) {
        if (!html) return html;
        let cleaned = html;
    ...
    parseWechatArticle: function(html) {
        try {
            ...
            if (contentEl) {
                ...
                result.content = this.cleanWechatContent(contentEl.innerHTML);
            }
    ...
    parseDouyin: function(html, url) {
        try {
            ...
            if (routerMatch) {
                try {
                    ...
                    const findItem = (obj, depth) => {
                        ...
                        if (obj.video && obj.video.play_addr && Array.isArray(obj.video.play_addr.url_list) && obj.video.play_addr.url_list.length > 0) {
                            return obj;
                        }
                        for (const k in obj) {
                    ...
                    if (videoItem) {
                        ...
                        // 视频地址：去除水印
                        // 两种地址格式：
                        // 1. https://aweme.snssdk.com/aweme/v1/playwm/?video_id=xxx -> 改为 /play/
                        // 2. https://xxx.douyinvod.com/.../playwm/... -> 改为 play/
                        if (videoItem.video && videoItem.video.play_addr && Array.isArray(videoItem.video.play_addr.url_list)) {
                            const playUrls = videoItem.video.play_addr.url_list
                                .filter(u => u && u.startsWith('http'))
                                .map(u => {
                                    let clean = u;
                                    if (clean.includes('/playwm/')) {
                                        clean = clean.replace('/playwm/', '/play/');
                                    }
                                    if (clean.includes('playwm%2F')) {
                                        clean = clean.replace('playwm%2F', 'play%2F');
                                    }
                                    if (clean.includes('aweme.snssdk.com') && clean.includes('/play/')) {
                                        clean = clean + '&watermark=0';
                                    }
                                    return clean;
                                });
                            result.videos = playUrls;
                        }
                    }
                } catch (e) {
            ...
            // 文案：保留完整描述（去掉末尾的话题标签后的纯文本作为内容）
            if (result.title) {
                const cleanDesc = result.title.replace(/(?:\s*#[^\s#]+#?)+$/g, '').trim();
                result.content = cleanDesc;
                result.contentHtml = '<p>' + cleanDesc.replace(/\n/g, '<br>') + '</p>';
