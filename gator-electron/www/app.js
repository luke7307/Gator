...
function loadSettings() {
    ...
    try {
        ...
        fullscreenReading = savedFullscreen !== null ? savedFullscreen === 'true' : true; // 默认开启全屏阅读
        updateFullscreenReadingSwitchUI();
    } catch(e) { fullscreenReading = true; }
}
...
function createCardHTML(item) {
    ...
    // 卡片流使用纯文本预览（紧凑格式），详情页使用 contentHtml 保留原格式
    ...
    const videoPreviewHtml = isVideoCard
        ...
        : `<p class="text-[13px] text-gray-500 line-clamp-3" style="line-height:1.8;">${contentHtml}</p>`;
    ...
    return `${openWrap}<div class="card-wrapper ${item.isXhsComment ? 'is-xhs-comment' : ''} ${item.isBilibiliComment ? 'is-bilibili-comment' : ''}" id="card-${item.id}" data-id="${item.id}" style="position:relative;overflow:hidden;border-radius:16px;">
        ${deleteBtnHtml}
        ${batchCheckbox}${exportRadio}
        <div class="card-content bg-white rounded-2xl p-4 card-shadow flex items-center gap-3 overflow-hidden" style="position:relative;z-index:2;${cardMinHeight ? `min-height:${cardMinHeight}px;` : ''}">
...
async function parseAndFillUrl(url, mode) {
    ...
    // 避免 platformName 与后续文字重复（如"网页网页"）
    const pName = platformName === '网页' ? '' : platformName;
    const statusMessages = useSnapshot 
        ? [`正在识别${platformName}链接...`, `正在加载${pName}网页...`, '正在截取页面...', '正在处理图片...', '即将完成...']
        : useSegmentSnapshot
            ? [`正在识别${platformName}链接...`, `正在加载${pName}网页...`, '正在分段截图...', '正在处理图片...', '即将完成...']
            : isVideoType
                ? [`正在识别${platformName}视频...`, `正在获取${pName}视频信息...`, '正在提取视频封面...', '正在处理视频信息...', '即将完成...']
                : [`正在识别${platformName}链接...`, `正在获取${pName}内容...`, '正在提取正文...', '正在处理排版...', '即将完成...'];
    let statusIdx = 0;
    const statusInterval = setInterval(() => {
        statusIdx = (statusIdx + 1) % statusMessages.length;
        updateExportProgress(Math.min(70, statusIdx * 17), statusMessages[statusIdx]);
    ...
    const modalTitle = useSnapshot 
        ? `${platformName}网页快照` 
        : useSegmentSnapshot 
            ? `${platformName}分段截图` 
            : platformName === typeLabel 
                ? `导入${platformName}` 
                : `导入${platformName}${typeLabel}`;
    showExportProgressModal(modalTitle);
    updateExportProgress(0, statusMessages[0]);
...
let fullscreenReading = localStorage.getItem('gator_fullscreen_reading') !== 'false'; // 默认开启全屏阅读
