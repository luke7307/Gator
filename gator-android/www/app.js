...
function createCardHTML(item) {
    ...
    if (item.isXhsComment && item.commentText) {
        const commentPreview = item.commentText.length > 20 ? item.commentText.substring(0, 20) + '...' : item.commentText;
        displayTitle = '【评论】' + commentPreview;
    }
    ...
    if (imagePreviewMode && currentLayout === 'masonry' && (firstImageUrl || shouldAutoCover) && !batchDeleteMode && !batchPinMode) {
        ...
        if (firstImageUrl) {
            ...
            return `<div class="card-wrapper masonry-item ${item.isXhsComment ? 'is-xhs-comment' : ''}" id="card-${item.id}" data-id="${item.id}" style="position:relative;overflow:hidden;border-radius:16px;" ${imgDataAttr}>
                ${deleteBtnHtml}
            ...
            // 自动配图卡片（无真实图片，使用标题配图）
            return `<div class="card-wrapper masonry-item ${item.isXhsComment ? 'is-xhs-comment' : ''}" id="card-${item.id}" data-id="${item.id}" style="position:relative;overflow:hidden;border-radius:16px;">
                ${deleteBtnHtml}
                ${batchCheckbox}${exportRadio}
                <div class="card-content bg-white rounded-2xl overflow-hidden card-shadow" style="position:relative;z-index:2;">
                    ${autoCoverHtml}
    ...
    return `${openWrap}<div class="card-wrapper ${item.isXhsComment ? 'is-xhs-comment' : ''}" id="card-${item.id}" data-id="${item.id}" style="position:relative;overflow:hidden;border-radius:16px;">
        ${deleteBtnHtml}
        ${batchCheckbox}${exportRadio}
        <div class="card-content bg-white rounded-2xl p-4 card-shadow flex items-center gap-3 overflow-hidden" style="position:relative;z-index:2;">
            <div class="flex-1 min-w-0 card-text-area" data-stack="${hasStackImages ? '1' : '0'}">
                <div class="flex items-start gap-1">
                    ${pinnedHtmlInline}
                    <h4 class="font-semibold text-gray-900 text-[15px] mb-0.5 flex-1 min-w-0 line-clamp-2">${titleHtml}</h4>
                </div>
...
function extractCommentInfoFromOcr(ocrText) {
    if (!ocrText) return { author: '', content: '', date: '' };
    ...
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        if (line.includes('评论了笔记') && i > 0) {
            author = lines[i - 1].replace(/[^\u4e00-\u9fa5a-zA-Z0-9]/g, '').trim();
    ...
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        if (line.includes('评论了笔记')) {
            inContent = true;
            continue;
        }
        if (inContent) {
            if (/\d{4}-\d{2}-\d{2}/.test(line)) break;
            if (line.includes('小红书') || line.includes('长按扫描')) break;
...
async function handleCommentImagePaste(dataUrl) {
    ...
    if (!parsed || parsed.platform !== 'xiaohongshu') return false;
    ...
    modal.innerHTML = `
        <div class="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl animate-[fadeIn_0.2s_ease-out]">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    <div class="font-bold text-lg">小红书评论分享图</div>
                    <div class="text-sm text-gray-500">识别到评论二维码</div>
                </div>
            </div>
...
async function importCommentWithOriginal(imgDataUrlEnc, urlEnc) {
    ...
    try {
        const [fullContent, ocrText] = await Promise.all([
            URL_PARSER.extractFullContent(url),
            ocrImageText(imgDataUrl)
        ]);
        ...
        const commentInfo = extractCommentInfoFromOcr(ocrText);
        
        let commentTextHtml = '';
        let commentTextMd = '';
        
        if (commentInfo.content) {
            const contentLines = commentInfo.content.split('\n').filter(l => l.trim());
            commentTextHtml = '<div style="margin-bottom:12px;">' + 
                (commentInfo.author ? `<div style="font-weight:600;font-size:15px;color:#1F2937;margin-bottom:4px;">${commentInfo.author}</div>` : '') +
                `<div style="font-size:14px;color:#374151;line-height:1.6;">${contentLines.join('<br>')}</div>` +
                '</div>';
            commentTextMd = (commentInfo.author ? `**${commentInfo.author}**\n\n` : '') + commentInfo.content + '\n\n';
        }
        
        const commentHtml = `
            <div style="margin-bottom:24px;padding-bottom:20px;border-bottom:1px solid #E5E7EB;">
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">
                    <span style="font-size:13px;font-weight:500;color:#6B7280;">💬 评论</span>
                    ${commentInfo.date ? `<span style="font-size:12px;color:#9CA3AF;">${commentInfo.date}</span>` : ''}
                </div>
                ${commentTextHtml}
                <div style="text-align:center;margin-top:${commentTextHtml ? '12px' : '0'};">
                    <img src="${imgDataUrl}" style="max-width:100%;max-height:70vh;object-fit:contain;border-radius:12px;box-shadow:0 2px 12px rgba(0,0,0,0.08);" />
                </div>
            </div>
        `;
        
        const commentMd = `💬 **评论**\n\n${commentTextMd}![评论图片](${imgDataUrl})\n\n---\n\n`;
        
        const finalContentHtml = commentHtml + (fullContent.contentHtml || '');
        const finalMd = commentMd + (fullContent.content || '');
        
        document.getElementById('inputEmojiBtn').textContent = fullContent.platformEmoji;
        currentEmoji = fullContent.platformEmoji;
        const commentTitle = commentInfo.content ? commentInfo.content.substring(0, 30) : (fullContent.title || '小红书笔记');
        document.getElementById('titleInput').value = '【评论】' + commentTitle;
        ...
        for (let i = 0; i < items.length; i++) {
            if (items[i].type.indexOf('image') !== -1) {
                ...
                reader.onload = async function(evt) {
                    ...
                    pasteArea.innerHTML = '<div style="text-align:center;"><img src="' + dataUrl + '" style="max-width:100%;max-height:200px;object-fit:contain;border-radius:8px;" /><div style="margin-top:8px;color:#8B5CF6;font-size:13px;">正在识别二维码...</div></div>';
                    const qrText = await scanQRCodeFromImage(dataUrl);
                    modal.remove();
                    if (qrText && URL_PARSER && URL_PARSER.isUrl(qrText)) {
                        const parsed = URL_PARSER.detectPlatform(qrText);
                        if (parsed && parsed.platform === 'xiaohongshu') {
                            showXhsCommentOptions(dataUrl, qrText);
                        } else {
...
function showXhsCommentOptions(dataUrl, qrText) {
    const modal = document.createElement('div');
    modal.className = 'fixed inset-0 bg-black/50 flex items-center justify-center z-[100] p-4';
    modal.innerHTML = `
        <div class="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl animate-[fadeIn_0.2s_ease-out]">
...
async function parseAndFillUrl(url, mode) {
    ...
    const statusMessages = useSnapshot 
        ? ['正在识别链接...', '正在加载网页...', '正在截取页面...', '正在处理图片...', '即将完成...']
        : ['正在识别链接...', '正在获取网页内容...', '正在提取正文...', '正在处理排版...', '即将完成...'];
    let statusIdx = 0;
    const statusInterval = setInterval(() => {
        statusIdx = (statusIdx + 1) % statusMessages.length;
        updateExportProgress(Math.min(70, statusIdx * 17), statusMessages[statusIdx]);
    ...
    showExportProgressModal(useSnapshot ? '网页快照导入' : '导入链接');
    updateExportProgress(0, statusMessages[0]);
...
function showDetailModal(id, targetImgIdx) {
    ...
    if (previewEl) {
        ...
        if (useRich) {
            ...
            const richImgs = previewEl.querySelectorAll('img');
...
function saveFullscreenChanges(mode) {
    ...
    if (fullContent !== oldContent) {
        ...
        // 对于富文本导入的文章，从 DOM 提取最新的 HTML 更新到 contentHtml
        // 而不是直接删除，这样修改后仍保留排版效果
        if (isRichImport) {
            const richContainer = contentEl.querySelector('.rich-imported-article');
            if (richContainer) {
...
function autoSaveDetail() {
    ...
    item.type = detailSelectedType;
    // 正文被编辑后，保留排版的 contentHtml 已与内容不一致，清除以避免展示旧排版
    if (newContent !== detailOriginalContent && item.contentHtml) {
...
function openFullscreenEditor() {
    ...
    if (useRich) {
        document.getElementById('fullscreenEditor').innerHTML = wrapEditorImages(currentItem.contentHtml);
    } else {
...
function _doCloseFullscreenEditor() {
    ...
    if (_isFromInputModal) {
        const title = document.getElementById('fullscreenTitle').value;
...
function openFullscreenEditorFromInput() {
    ...
    const modal = document.getElementById('fullscreenEditorModal');
    document.getElementById('fullscreenEmojiBtn').textContent = detailEmoji;
    document.getElementById('fullscreenTitle').value = inputTitle;
    if (useRich) {
        // 优先渲染保留排版的原始 HTML
        document.getElementById('fullscreenEditor').innerHTML = wrapEditorImages(importedRich.contentHtml);
    } else if (inputContent.trim()) {
        document.getElementById('fullscreenEditor').innerHTML = wrapEditorImages(markdownToHtml(inputContent));
        document.getElementById('fullscreenEditor').querySelectorAll('div[data-task] span').forEach(span => {
            span.contentEditable = 'true';
        });
    } else {
...
function wrapEditorImages(html) {
    return html.replace(/<img([^>]*)>/gi, function(match, attrs) {
        ...
        const imgStyle = (origStyle ? origStyle + '; ' : '') + 'max-width:100%; border-radius:12px; display:block; transition:box-shadow 0.2s ease; -webkit-user-drag:none; user-select:none; pointer-events:none;';
        return (
            '<div data-editor-img-container contenteditable="false"' + origAttr + ' style="position:relative; display:block; margin:10px 0; line-height:0; outline:2px solid transparent; outline-offset:3px; border-radius:12px; transition:outline-color 0.15s ease, box-shadow 0.2s ease;">' +
                '<img src="' + src + '" data-orig-img="1" style="' + imgStyle + '" draggable="false" decoding="async">' +
...
function testExportAll() {
    const target = items.find(i => i.title.includes('睡眠质量优化清单'));
    if (!target) {
        showToast('请先添加示例数据');
        return;
    }
    handleExportFormat('longimage', target);
    setTimeout(() => {
        handleExportFormat('pdf', target);
        setTimeout(() => {
            handleExportFormat('mindmap', target);
        }, 1500);
    }, 1500);
}
